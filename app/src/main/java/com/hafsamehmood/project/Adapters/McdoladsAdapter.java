...
public class McdoladsAdapter extends RecyclerView.Adapter<McdoladsAdapter.ViewHolder>{
    private List<Modelofallrecipes> list_secret;
    Context context;

    public McdoladsAdapter(List<Modelofallrecipes> list_secret, Context context) {
    ...
    @NonNull
    @Override
    public McdoladsAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.item_all_foods,parent,false);
        return new McdoladsAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull McdoladsAdapter.ViewHolder holder, int position) {
