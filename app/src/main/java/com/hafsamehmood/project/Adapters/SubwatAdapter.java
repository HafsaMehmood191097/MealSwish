...
public class SubwatAdapter extends RecyclerView.Adapter<SubwatAdapter.ViewHolder> {
    private List<Modelofallrecipes> list_secret;
    Context context;

    public SubwatAdapter(List<Modelofallrecipes> list_secret, Context context) {
        this.list_secret = list_secret;
        this.context = context;
    }
    @NonNull
    @Override
    public SubwatAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.item_all_foods,parent,false);
        return new SubwatAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull SubwatAdapter.ViewHolder holder, int position) {
